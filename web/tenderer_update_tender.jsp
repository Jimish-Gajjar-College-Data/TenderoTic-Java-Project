        ...
        <script>
            $(document).ready(function () {
                $('#ul1 li').click(function ()
                {
                    var val1 = $(this).text();
                    $.ajax({
                        url: "admin_block_tm_ajax.jsp", //this is my servlet
                        data: "input=" + val1,
                ...
                <div class="content-main">
                    ...
                    <div class="blank"> 
                        <div class="blank-page">
                            <form action="tm_show_winner.jsp" id="myForm">
                                <%
                                    int count_tende = 0;
                                    int flag = 0;
                                    PreparedStatement result_decler = con.prepareStatement("select t.* ,p.name from tender_info t,result_tender r,fill_tender f, product p where t.tender_id=f.tender_id and f.tender_id not IN(r.tender_id) and f.tenderer_id='" + Tenderer_id + "' and  t.product_id = p.product_id");
                                    ResultSet rs_result_decler = result_decler.executeQuery();
                                    while (rs_result_decler.next()) {
                                        int tender_id = rs_result_decler.getInt("tender_id");
                                    ...
                                    <h6>                                    
                                        <table id="output">
                                            ...
                                            <%
                                                ...
                                                PreparedStatement tender_display = con.prepareStatement("select t.* ,p.name from tender_info t,result_tender r,fill_tender f, product p where t.tender_id=f.tender_id and f.tender_id not IN(r.tender_id) and f.tenderer_id='" + Tenderer_id + "' and  t.product_id = p.product_id limit 0,5");
                                                ResultSet rs_tender_display = tender_display.executeQuery();
                                                while (rs_tender_display.next()) {
                                                    int tender_id1 = rs_tender_display.getInt("tender_id");

                                            %>
                                            <tr>
                                                <td><%=rs_tender_display.getString("t.title")%></td>
                                                <td><%=rs_tender_display.getString("t.description")%></td>
                                                <td><%=rs_tender_display.getString("p.name")%></td>
                                                <td><%=rs_tender_display.getString("t.price")%></td>
                                                <td><%=rs_tender_display.getString("t.type_of_work")%></td>
                                                <td><%=rs_tender_display.getDate("t.reg_date")%></td>
                                                <td><%=rs_tender_display.getDate("t.verification_date")%></td>
                                                <td><%=rs_tender_display.getDate("t.result_dec_date")%></td>
                                                <td>                                                
